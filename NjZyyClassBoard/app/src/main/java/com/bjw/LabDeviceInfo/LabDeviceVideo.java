...
public class LabDeviceVideo extends Activity {
    Intent intent1;
    private final String Tag = LabDeviceVideo.class.getSimpleName();
    ...
    private LabDeviceMainBean labDeviceMainBean;
...
//            final int playprogress=mMediaPlayer.getCurrentPosition();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_labroom_playvideo);
        findView();
        ...
        labDeviceMainBean=new LabDeviceMainBean(LabID,LabDeviceId,LabDeviceName,LabDeviceImageUrl,LabDeviceSupplier,LabDeviceIntroduct,LabDeviceVideoUrl);
        TvLabDeviceName.setText("设备名称: "+labDeviceMainBean.getLab_device_name());
        if(labDeviceMainBean.getLab_device_supplier().equals("")||labDeviceMainBean.getLab_device_supplier()==null)
        {
            TvLabDeviceResource.setText("设备来源: " + "未知");
        }
        else {
            TvLabDeviceResource.setText("设备来源: " + labDeviceMainBean.getLab_device_supplier());
        }
        if(labDeviceMainBean.getLab_device_introduct()==null)
        {
            TvLabDeviceIntroduct.setText("设备简介: "+"无");
        }
        else {
            TvLabDeviceIntroduct.setText("设备简介: "+labDeviceMainBean.getLab_device_introduct());
        }
        if(LabDeviceImageUrl!=null) {
            Glide.with(getBaseContext()).load(labDeviceMainBean.getLab_device_image_url()).into(IvLabDeviceImage);
        }
        ...
        if(count==0)
        {
            ...
            intent1.putExtra("url",labDeviceMainBean.getLab_device_video_url());
    ...
    private class PlayerVideoThread extends Thread{
        ...
        public void run(){
            try {
                ...
                if(count==0)
                {
//              Uri uri = Uri.parse(labDeviceMainBean.getLab_device_video_url());
                Uri uri = Uri.parse("android.resource://com.bjw.ComAssistant/"+R.raw.ansen);
                mMediaPlayer.setDataSource(LabDeviceVideo.this,uri);
//                mMediaPlayer.setDataSource("/mnt/internal_sd/班牌视频文件/test.mp4");
                }
